...
if (!process.env.OPENAI_API_KEY) {
  ...
  console.error('Please add OPENAI_API_KEY=sk-ant-api03-your-key-here to your .env file');
  process.exit(1);
}
...
app.post('/start-ai-agent', async (req, res) => {
  ...
  // Additional validation for Anthropic platform
  if (platform === 'openai' && !process.env.OPENAI_API_KEY) {
    res.status(500).json({ 
  ...
  try {
    ...
    if (errorMessage.includes('invalid x-api-key') || errorMessage.includes('authentication_error')) {
      res.status(401).json({ 
        ...
        reason: 'Please check your OPENAI_API_KEY in the .env file' 
      });
    } else {
